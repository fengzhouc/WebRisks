...
public class HttpListener implements IHttpListener, IMessageEditorController {
    ...
    public HttpListener() {
        try {
            ...
            BurpExtender.callbacks.printError("[HttpListener.Md5.getInstance] " + e.getMessage());
        }
    ...
    public void processHttpMessage(int toolFlag, boolean messageIsRequest, IHttpRequestResponse messageInfo) {
        ...
        if (!messageIsRequest && MainPanel.intercepts.containsValue(toolFlag)) {
            ...
            if (MainPanel.kg && m_host) { //是否开启插件，开启后匹配设置的domain才会进行扫描
                ...
                BurpExtender.callbacks.printOutput("inCache " + String.format("method_%s_url_%s_status_%d", BurpReqRespTools.getMethod(messageInfo), url, BurpReqRespTools.getStatus(messageInfo)));
            }
        }
    }
