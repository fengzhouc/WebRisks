...
public class Json3rd extends VulTaskImpl {
    ...
    private Json3rd(HttpRequestResponseWithMarkers requestResponse) {
...
class Json3rdCallback implements Callback {
    ...
    public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
        ...
        if (respBpdy.contains("fastjson")) {
            message = "发现使用fastjson，可以深度探测下是否存在反序列化";
        }else if (respBpdy.contains("jackson")) {
            message = "发现使用jackson，可以深度探测下是否存在反序列化";
        }else if (respBpdy.contains("gson")) {
            message = "发现使用gson，可以深度探测下是否存在反序列化";
        }
        // 记录日志
        MainPanel.logAdd(
            requestResponse, 
