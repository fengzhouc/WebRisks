...
public class JsonCsrf extends VulTaskImpl {
    ...
    private JsonCsrf(HttpRequestResponseWithMarkers requestResponse) {
    ...
    public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
        ...
        if (response.isSuccessful()){
            ...
            if (BurpReqRespTools.getStatus(requestResponse) == BurpReqRespTools.getStatus(vulTask.requestResponse)
                ...
                message = "发现请求体是Json数据的CSRF攻击，即后端没有限制Content-Type及无使用CsrfToken，转换成表单形式进行攻击";
            }
        }
        // 记录日志
        MainPanel.logAdd(
